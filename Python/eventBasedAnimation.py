...
def run(initFn=None, mouseFn=None, rightMouseFn=None, keyFn=None, stepFn=None, 
        drawFn=None,
    ...
    def onRightMousePressedWrapper(event):
        if (not data.isRunning): return        
        callAnimationFn(rightMouseFn, event, data.userData)
    ...
    def runAnimation():
        ...
        bindings = [("<Motion>"          , onMouseMovedWrapper),
                    ...
                    ("<Button-3>"        , onRightMousePressedWrapper)
                    ]
...
class Animation(object):
    ...
    def onRightMouse(self, event): pass
    ...
    def run(self):
        ...
        def rightMouseFn(event, data): self.onRightMouse(event)
        ...
        run(initFn=initFn, mouseFn=mouseFn, rightMouseFn=rightMouseFn, 
            keyFn=keyFn, stepFn=stepFn,
